...
const ChartStyle = ({ id, config }: { id: string; config: ChartConfig }) => {
  ...
  const sanitizeColor = (color: string): string => {
    ...
    const validColorRegex = /^(#[0-9a-fA-F]{3,6}|rgb\([^)]+\)|rgba\([^)]+\)|hsl\([^)]+\)|hsla\([^)]+\)|[a-zA-Z]+)$/
  ...
  const sanitizeKey = (key: string): string => {
    ...
    return validKeyRegex.test(key) ? key : ''
  }
  ...
  const sanitizeId = (id: string): string => {
    ...
    return validIdRegex.test(id) ? id : ''
  }

  const sanitizedId = sanitizeId(id)
  if (!sanitizedId) {
    console.warn('Invalid chart ID provided, skipping styles')
    return null
  }
  ...
  const cssRules = Object.entries(THEMES)
    .map(([theme, prefix]) => {
      ...
      return colorRules ? `${prefix} [data-chart="${sanitizedId}"] {\n${colorRules}\n}` : null
    })
    .filter(Boolean)
    .join('\n')
